...
#ifndef OPENCVTEST_CLASSIFIER_H
#define OPENCVTEST_CLASSIFIER_H
...
std::vector<cv::Mat> CaffeModel::forwardPass(const cv::Mat &img, int layer_nr)
{
    Blob<float>* input_layer = net_->input_blobs()[0];
...
void CaffeModel::SetMean(const string& mean_file) {
 ...
 * layer. */
void CaffeModel::WrapInputLayer(std::vector<cv::Mat>* input_channels) {
    Blob<float>* input_layer = net_->input_blobs()[0];
...
void CaffeModel::Preprocess(const cv::Mat& img,
                            std::vector<cv::Mat>* input_channels) {
